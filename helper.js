function advancedEnclosersExtract(str) {
  ...
  function proceed() {
    ...
    while (i < fatherRunner) {
      ...
      if (value === undefined) break //i will increment out of bound, so we stop loop here.
...
function areEnclosersBalanced(str) {
...
function reFine(text) {
  if (Array.isArray(text)) return text.map(e => e = e.trim()).filter(e => e !== '')
  else if (typeof text === 'string') return text.trim()
}

function paraCore(text) {
  const regPara = /\n/gm
...
function attachEnclosers(text) {
  ...
  if (Array.isArray(text)) {
    while (i < text.length) {
      ...
      else //console.log('no openers found')
      i++
    }
...
function nextValueFn(t, i) {
  return t[i + 1] ? { value: t[i + 1], index: i + 1 } : { value: '', index: void(0) }
}

function escaper(text, lookOut, mode, prevIndexes) {
  if (mode === 'begin') {
    let escaperValue
    if (Array.isArray(text)) { //input may be array
      ...
      text = text.map(v => v.replaceAll(lookOut, (match) => valueEscaped))
    } else {
      ...
      const valueEscaped = lookOut[0] + escaperValue + lookOut.slice(1)
      text = text.replaceAll(lookOut, (match) => valueEscaped)
    }
...
module.exports = { advancedEnclosersExtract, areEnclosersBalanced, reFine, paraCore, attachEnclosers, nextValueFn, escaper }